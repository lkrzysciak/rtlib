...
#define __static_pool_methods_c(type_t, member_t, pool_capacity)                    \
    ...
    {                                                                               \
        ...
        {                                                                           \
            self->nodes[block_no].storage.next = &self->nodes[block_no + 1];        \
        }                                                                           \
        self->last_free_block               = &self->nodes[pool_capacity - 1];      \
        self->last_free_block->storage.next = NULL;                                 \
    }                                                                               \
                                                                                    \
    void type_t##_Destruct(type_t * const self)                                     \
    {                                                                               \
    ...
    {                                                                               \
        ...
        {                                                                           \
            ...
            self->first_free_block          = self->first_free_block->storage.next; \
            if(!self->first_free_block)                                             \
            {                                                                       \
                self->last_free_block = NULL;                                       \
            }                                                                       \
            return &to_return_block->storage.value;                                 \
        }                                                                           \
        else                                                                        \
        {                                                                           \
            return NULL;                                                            \
    ...
    {                                                                               \
        ...
        node->storage.next     = self->first_free_block;                            \
        self->first_free_block = node;                                              \
        if(!self->last_free_block)                                                  \
        {                                                                           \
