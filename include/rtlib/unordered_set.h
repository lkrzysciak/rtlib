...
#ifndef RTLIB_UNORDERED_SET_KEY_EQUAL
#define RTLIB_UNORDERED_SET_KEY_EQUAL(key_t, a, b) (key_t##_Compare((const key_t *)(a), (const key_t *)(b)) == 0)
#endif
...
#define __hash_table_methods_h(container_t, key_t)                                              \
    ...
    const key_t * container_t##_FindPtr(container_t * const self, key_t data);                  \
    container_t##_Iterator container_t##_Find(container_t * const self, key_t data);            \
    bool container_t##_Contains(container_t * const self, key_t data);                          \
                                                                                                \
...
#define __static_hash_table_methods_c(container_t, key_t, container_capacity)                                            \
    ...
    {                                                                                                                    \
        ...
        container_t##_node * first_tombstone = NULL;                                                                     \
                                                                                                                         \
        /* Fast path: check the first slot without loop overhead. */                                                     \
        {                                                                                                                \
            ...
            {                                                                                                            \
                if(RTLIB_UNORDERED_SET_KEY_EQUAL(key_t, &data, &slot0->key))                                             \
                {                                                                                                        \
        ...
        {                                                                                                                \
            ...
            {                                                                                                            \
                if(RTLIB_UNORDERED_SET_KEY_EQUAL(key_t, &data, &slot->key))                                              \
                {                                                                                                        \
                    return ELEMENT_EXISTS;                                                                               \
                }                                                                                                        \
            }                                                                                                            \
    ...
    {                                                                                                                    \
        ...
        if(slot0->state == __##container_t##_STATE_OCCUPIED &&                                                           \
           RTLIB_UNORDERED_SET_KEY_EQUAL(key_t, &data, &slot0->key))                                                     \
        {                                                                                                                \
            return (const key_t *)&slot0->key;                                                                           \
        }                                                                                                                \
                                                                                                                         \
        index = (index + 1u) & mask;                                                                                     \
        for(unsigned int probe = 1; probe < (unsigned int)__##container_t##_TABLE_SIZE; ++probe)                         \
        {                                                                                                                \
            ...
            if(slot->state == __##container_t##_STATE_OCCUPIED &&                                                        \
               RTLIB_UNORDERED_SET_KEY_EQUAL(key_t, &data, &slot->key))                                                  \
            {                                                                                                            \
                return (const key_t *)&slot->key;                                                                        \
            }                                                                                                            \
            index = (index + 1u) & mask;                                                                                 \
        }                                                                                                                \
        return NULL;                                                                                                     \
    ...
    {                                                                                                                    \
        ...
        it.index                = (size_t)__##container_t##_TABLE_SIZE;                                                  \
        const key_t * found_ptr = container_t##_FindPtr(self, data);                                                     \
        if(found_ptr)                                                                                                    \
        {                                                                                                                \
            it.node  = (container_t##_node *)found_ptr;                                                                  \
            it.index = (size_t)(it.node - self->nodes_table);                                                            \
        }                                                                                                                \
        return it;                                                                                                       \
    }                                                                                                                    \
    bool container_t##_Contains(container_t * const self, key_t data)                                                    \
    {                                                                                                                    \
        return container_t##_FindPtr(self, data) != NULL;                                                                \
    }                                                                                                                    \
                                                                                                                         \
    void container_t##_Clear(container_t * const self)                                                                   \
    {                                                                                                                    \
...
#define __custom_allocator_hash_table_methods_c(container_t, key_t, allocator_t)                                       \
    ...
    {                                                                                                                  \
        ...
        __##container_t##_STATE_TOMBSTONE = 2u                                                                         \
                                                                                                                       ...
                                                                                                                       \
    static bool __##container_t##_InsertKeyToHashTable(container_t * const self, const key_t * key,                    \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            if(slot0->state == __##container_t##_STATE_OCCUPIED)                                                       \
            {                                                                                                          \
                if(RTLIB_UNORDERED_SET_KEY_EQUAL(key_t, key, &slot0->key))                                             \
                {                                                                                                      \
                    return false;                                                                                      \
                }                                                                                                      \
            }                                                                                                          \
        ...
        {                                                                                                              \
            ...
            if(slot->state == __##container_t##_STATE_OCCUPIED)                                                        \
            {                                                                                                          \
                if(RTLIB_UNORDERED_SET_KEY_EQUAL(key_t, key, &slot->key))                                              \
                {                                                                                                      \
                    return false;                                                                                      \
                }                                                                                                      \
            }                                                                                                          \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            if(self->nodes_table[i].state == __##container_t##_STATE_OCCUPIED)                                         \
            {                                                                                                          \
                (void)__##container_t##_InsertKeyToHashTable(self, (const key_t *)&self->nodes_table[i].key,           \
    ...
    {                                                                                                                  \
        assert(self);                                                                                                  \
                                                                                                                       \
        /* Linear probing gets expensive quickly as load factor approaches 1.0; keep ~<= 0.5. */                       \
...
#define dynamic_unordered_set_impl(container_t, key_t)   \
